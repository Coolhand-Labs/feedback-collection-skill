...
├── skills/
│   ├── shared/
│   │   └── common.md            # Prose injected into every SKILL.md by bin/build-skills
...
Each skill's `SKILL.md` is generated. Do not edit it by hand. `bin/build-skills` assembles it from two sources:

- `skills/<name>/SKILL.src.md` is the hand-edited body and frontmatter for that skill.
- `skills/shared/common.md` is prose injected into every skill, right after its frontmatter. Currently the canonical pointers to `source_apis.yml` and `detection-patterns/providers.yml`.
...
To change a skill: edit its `SKILL.src.md` (or `skills/shared/common.md` for text shared by all three), run `bin/build-skills`, and commit the regenerated `SKILL.md` alongside your source edit.
