...
├── .github/workflows/
│   ├── build-skills.yml         # CI: runs bin/build-skills --check on skill-source PRs
...
├── bin/
│   ├── build-skills             # Ruby stdlib - builds SKILL.md from SKILL.src.md + shared/common.md
...
├── skills/
│   ├── shared/
│   │   └── common.md            # Prose injected into skills that opt in (shared_common: true)
│   ├── feedback-collection/     # Planner: scans, designs strategy, dispatches
│   │   ├── SKILL.src.md         # Hand-edited source
│   │   ├── SKILL.md             # GENERATED by bin/build-skills - do not edit
...
│   │       ├── chained-partial-feedback.md       # Chain-deletion + offset-staleness semantics
...
│   └── wildcard-tool/           # Standalone: consults on adding a wildcard "complaint box" tool
│       ├── SKILL.src.md         # Hand-edited source (opts out of common.md)
│       └── SKILL.md             # GENERATED by bin/build-skills - do not edit
├── CHANGELOG.md
├── AGENTS.md                    # This file
├── LICENSE                      # Apache-2.0
...
**One plugin, four skills.** The plugin (`feedback-collection`) ships four skills under `skills/`. The planner (`feedback-collection`) is the user-facing entry point and dispatches to either implementation skill (`coolhand-integration` or `self-hosted-feedback`) at its Phase 5. `wildcard-tool` is independently invocable and stands outside that dispatch trio — it consults on adding a "complaint box" tool to any agentic project, with no dependency on the planner having run. None of the four skills is independently useful in isolation from the plugin — install the plugin and all four load together.
...
## Building skills (`SKILL.md` is generated)

Each skill's `SKILL.md` is generated. Do not edit it by hand. `bin/build-skills` assembles it from:

- `skills/<name>/SKILL.src.md` — the hand-edited body and frontmatter for that skill.
- `skills/shared/common.md` — prose injected into skills that opt in, right after their frontmatter. Currently the canonical pointers to `source_apis.yml` and `detection-patterns/providers.yml`.

**`common.md` injection is opt-in.** Add `shared_common: true` to a `SKILL.src.md`'s frontmatter to have `common.md` injected into its build. Skills whose content has nothing to do with LLM provider detection (e.g. `wildcard-tool`) omit the flag and get their body only — no need to write `shared_common: false` explicitly, absence is the default. The generated banner comment reflects which case applies, so a generated `SKILL.md` always states its true sources.

`common.md` is injected after the YAML frontmatter so the frontmatter stays first; a leading `---` block is required for skill discovery.

To change a skill: edit its `SKILL.src.md` (or `skills/shared/common.md` for text shared by the skills that opt in), run `bin/build-skills`, and commit the regenerated `SKILL.md` alongside your source edit.

```
bin/build-skills --self-test   # prove the script works
bin/build-skills               # rebuild all SKILL.md
bin/build-skills --check       # verify committed SKILL.md is in sync
```

CI runs `--self-test` and `--check` on every PR touching skill sources; a stale `SKILL.md` fails the build. Keep the "Rationalizations to resist" tables in each `SKILL.src.md` (skill-local by design); do not move them into `common.md`.

Throughout the rest of this document, rules that say "edit SKILL.md" mean edit `SKILL.src.md` (the source); `SKILL.md` is the build output.
...
Scope is intentionally narrow: markdown only. JSON/YAML references (the OpenAPI snapshot, the detection-patterns YAML) are excluded because they may contain third-party example URLs that aren't ours to monitor. URLs in `README.md`, top-level `AGENTS.md`, and `CHANGELOG.md` are human-facing and not consulted at runtime — also out of scope here. The trusted-URL check (`bin/check-trusted-urls`) covers those instead.
