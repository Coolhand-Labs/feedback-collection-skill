...
Run `git diff origin/main...HEAD` to get the current branch diff, and read `AGENTS.md` for this repo's conventions before judging anything. Review the diff for:
...
- **Frontmatter conventions.** `description` in `SKILL.src.md` must contain triggering conditions only (symptoms, slash-command aliases, package names) — not a workflow/phase summary (see AGENTS.md "SKILL.md description field"). `shared_common: true` should be present only on `SKILL.src.md` files that actually reference `source_apis.yml` / `detection-patterns/providers.yml`; flag it on skills that don't need that prose, or missing on ones that do.
- **Inline content that belongs in YAML.** Per-provider or per-language snippets (SDK import patterns, extraction code, host names) added directly into a `SKILL.src.md` instead of `detection-patterns/providers.yml` or `source_apis.yml` (see AGENTS.md "No language-specific instructions inline in SKILL.md").
...
- **PR scope.** Note (don't block on) whether the diff bundles unrelated concerns that AGENTS.md's "Commits and PRs" section says should be separate PRs.
