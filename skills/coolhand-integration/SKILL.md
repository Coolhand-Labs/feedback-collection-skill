...
- The best available matching field (`llm_request_log_id` if SDK auto-monitor is on; otherwise `llm_provider_unique_id` — see `../feedback-collection/source_apis.yml` for the field name per provider and `detection-patterns/providers.yml` → `request_id_extraction` for per-language snippets).
