...
For questions about how Coolhand behaves, fetch the Coolhand Help Center index at `https://coolhandlabs.com/help.md` and read the relevant article rather than answering from memory. The `wildcard` command (aliases `complaint-box`, `report-blocker`) ships in `coolhand-cli`; use the latest (`npx coolhand-cli@latest wildcard`) and run `coolhand help wildcard` for its flags.
...
The wildcard tool takes two parameters the agent fills in. Keep it as small
as possible.
...
- `original_output` (string, required). The last user request or turn that
  was sent before the agent hit the problem, pasted by the agent. This
  gives the developer the context of what the agent was asked to do
  alongside the complaint. Do not ask the agent for its thinking or
  reasoning chain, and do not have the runtime capture one. Describe the
  parameter to the agent in the tool's schema with an instruction such as
  "Paste the last user request/turn that was sent before the error was hit."
...
3. **Minimum wire format if no Coolhand SDK or wiring is present.**
   ...
   the last user turn, and the agent name) and let the
